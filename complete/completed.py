...
class Commander(Node):
    ...
    def make_face(self, making, feeling):
        ...
        instruction = "Please express" + feeling + making

        # テキストからの画像生成の実行
        response = client.images.generate(
            model="dall-e-3",
            # cute cat eared maid character in a cafe"
            prompt=instruction,
            size="1024x1024",
            quality="standard",
            n=1,
        ...
        image = PIL.Image.open(image_stream)   # from PIL import Image 重複エラー
    ...
    def encode_image(self, input_audio):
            ...
            messages=[
                {
                  ...
                  "content": [
                      ...
                      {"type": "text", "text": '''Create a single prompt that follows the instruction according to the following template throughout these images

                        template = “[color]\n[pattern]\n[features]”

                        [color]= Color designation, color type, color intensity, etc.
                        [pattern]= The type of pattern, balance of placement, size, etc.
                        [features]= Image features, eye shape, nose shape, mouth shape, face shape, hair shape, hair color, clothing shape, clothing color, clothing pattern, clothing decoration, etc.
                        '''},
                      #   {"type": "text", "text": "These are face. Read the features from these images and create prompts that can generate images in a format similar to these."},
        ...
        img, txt = self.make_face(data, feeling)
            ...
            **mini_pupper_getup!!**    
            ...
            with gr.Tab("voice"):
                with gr.Row():
                    ...
                    msg2 = gr.Textbox(lines=1, label="your instruction")

                with gr.Row():
                    gr.Image(
                        type="pil", value=files[12], label="example_sad")

                    # type="pil", label="makeimage"
                    make_image = gr.Image(type="filepath", label="face")

                    gr.Image(
                        type="pil", value=files[14], label="example_smile")
                with gr.Row():
                    makeface = gr.components.Audio(
                        source="microphone", autoplay=True, type="filepath", label="makeface")
                    msg3 = gr.Textbox(lines=1, label="your prompt")
